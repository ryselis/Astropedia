...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" 
  ...
  <!-- Output the img tag formatted to display a diagram.  
        ...
        <xsl:for-each select="$element/publish:partInfo">
          ...
          <xsl:if test="@shape = 'rect'">
            ...
            <area shape="rect">
              ...
              <xsl:choose>
                <xsl:when test="@publish:nohref = 'true'">
                  <xsl:attribute name="nohref">
                    <xsl:value-of select="@publish:nohref"/>
                  </xsl:attribute>
                </xsl:when>
                <xsl:otherwise>
  ...
  <xsl:template name="getType" match="publish:properties/publish:property[@canonicalName='type']">
  ...
  <xsl:template name="createLinkToElement">
