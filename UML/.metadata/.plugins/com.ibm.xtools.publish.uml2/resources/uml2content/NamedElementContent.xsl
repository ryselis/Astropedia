...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- ======================================================================================== -->
	<xsl:template
		match="packagedElement | region | node | nestedNode | containedNode | nestedClassifier | ownedPort | occurrence | 
		ownedBehavior | generalization | subvertex | edge | containedEdge | lifeline | message | configuration | 
        target | result | argument | fragment | realization | interfaceRealization | extensionPoint | 
        effect | trigger | transition | implementation | substitution | packageImport | elementImport | include | extend | 
        manifestation | deployment | ownedConnector | entry | exit | doActivity | ownedParameteredElement | ownedUseCase | 
        group | ownedReception | ownedLiteral | slot | value | subpartition"
		mode="NamedElement">
        ...
        <xsl:if test="element-available('redirect:open')">
        	<redirect:open file="{$elementContentFullPathFileName}" />
        </xsl:if>		
        ...
        <xsl:if test="element-available('redirect:close')">
        	<redirect:close file="{$elementContentFullPathFileName}" />
        </xsl:if>
	</xsl:template>
	...
	<!-- There are 3 kinds of comments that are currently used by UML model:
		...
		owns the comment).
		3) urls (<<URL>> and <<LINK>> stereotyped comments)
	...
	<!-- Creates html contents for literals -->
	<xsl:template name="createSlotsAndValuesInfo">
		<xsl:param name="mode" select="''" />
		<xsl:if test="count(slot) &gt; 0">
			<xsl:choose>
				...
				<xsl:when test="$mode = 'link'">
					<publish:bookmarklink>
						<xsl:attribute name="name">
							<xsl:copy-of select="$slotsAndValuesNavName" />
						</xsl:attribute>
						<xsl:attribute name="link">
							<xsl:value-of
								select="concat('#', $slotsAndValuesNavName)" />
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:copy-of select="$slotsAndValuesNavName" />
						</xsl:attribute>
				...
				<xsl:when test="$mode = 'summary'">
					...
					<table class="SummaryTable" border="1">
						<xsl:call-template name="createTableTitle">
							<xsl:with-param name="title"
								select="$slotsAndValuesTableTitle" />
							<xsl:with-param name="colspan" select="3" />
						</xsl:call-template>
						<!-- Loop for each element creating an entry in the table -->
						<xsl:for-each select="slot">
							<xsl:variable name="definingFeature" select="publish:properties/publish:property[@canonicalName='definingFeature']" />
							<!-- 
							<xsl:variable name="attributeId" select="substring($definingFeature/@anchor, 2)" />
 	        				<xsl:variable name="attribute" select="key('EnumerationAttributesByIds', $attributeId)"/>
 	        				<xsl:variable name="enumerationLiteral" select="$attribute/.."/>
 	        				 -->
 	        				<xsl:variable name="value" select="value"/>
 	        				<xsl:variable name="valueProperty" select="$value/publish:properties/publish:property[@canonicalName='value']" />
 	        				<xsl:variable name="valueText">
 	        					<xsl:choose>
 	        						<xsl:when test="$valueProperty/@publish:value != ''">
 	        							<xsl:value-of select="$valueProperty/@publish:value"/>
 	        						</xsl:when>
 	        						<xsl:otherwise>
 				        				<xsl:variable name="bodyProperty" select="$value/publish:properties/publish:property[@canonicalName='body']" />
 	        							<xsl:value-of select="$bodyProperty/@publish:value"/>
 	        						</xsl:otherwise>
 	        					</xsl:choose>
 	        				</xsl:variable>
					        <tr>
					            <td>
					                <xsl:call-template name="outputImageAndTextLinkHTML">
					                    ...
					                    <xsl:with-param name="text" select="$definingFeature/@publish:value"/> 
					                </xsl:call-template>
					            </td>
					            <!-- 
					            <td>
				                    <xsl:value-of select="$attribute/@publish:type"/>
					            </td>
					             -->
					            <td>
					                <xsl:call-template name="outputImageAndTextLinkHTML">
	...
	<!-- Creates link to Packaged Elements summary -->
	<xsl:template name="createPackagedElementsInfo">
		<xsl:param name="mode" select="''" />
		<xsl:if test="count(packagedElement[@xsi:type='uml:Component' or @xsi:type='uml:Class']) &gt; 0">
			<xsl:choose>
				<xsl:when test="$mode = 'link'">
					<publish:bookmarklink>
						<xsl:attribute name="name">
							<xsl:copy-of
								select="$packagedElementsNavName" />
						</xsl:attribute>
						<!-- DON'T FORMAT THE LINE BELOW!!! -->
						<xsl:attribute name="link">
							<xsl:value-of
								select="concat('#', 'packagedElementsSummary')" />
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:copy-of
								select="$packagedElementsNavName" />
				...
				<xsl:when test="$mode = 'summary'">
					<p />
					<!-- Create an 'Packaged Elements' table for the content HTML -->
					<!-- Begin a table for the contained elements -->
					<table class="SummaryTable" border="1">
						<tr>
							<td class="SummaryTitle" colspan="2">
								<a name="packagedElementsSummary">
									<xsl:copy-of
										select="$packagedElementsNavName" />
								</a>
							</td>
						...
						<!-- Loop for each attribute creating an entry in the table -->
						<xsl:for-each select="packagedElement[@xsi:type='uml:Component' or @xsi:type='uml:Class']">
							<xsl:sort select="@name" />
							<tr>
								<td>
									<xsl:call-template
						...
						<!-- Close out the packaged elements table -->
					</table>
				</xsl:when>
			</xsl:choose>
		</xsl:if>
	...
	<!-- Creates link to Nested Classifiers summary -->
	<xsl:template name="createNestedNodesInfo">
		<xsl:param name="mode" select="''" />
		<xsl:if test="count(nestedNode) &gt; 0">
			<xsl:choose>
				<xsl:when test="$mode = 'link'">
					<publish:bookmarklink>
						<xsl:attribute name="name">
							<xsl:copy-of
								select="$nestedNodesNavName" />
						</xsl:attribute>
						<!-- DON'T FORMAT THE LINE BELOW!!! -->
						<xsl:attribute name="link">
							<xsl:value-of
								select="concat('#', 'nestedNodesSummary')" />
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:copy-of
								select="$nestedNodesNavName" />
				...
				<xsl:when test="$mode = 'summary'">
					<p />
					<!-- Create an 'Nested nodes' table for the content HTML -->
					<!-- Begin a table for the contained elements -->
					<table class="SummaryTable" border="1">
						<tr>
							<td class="SummaryTitle" colspan="2">
								<a name="nestedNodesSummary">
									<xsl:copy-of
										select="$nestedNodesNavName" />
								</a>
							</td>
						...
						<!-- Loop for each attribute creating an entry in the table -->
						<xsl:for-each select="nestedNode">
							<xsl:sort select="@name" />
							<tr>
								<td>
									<xsl:call-template
					...
					<table class="SummaryTable" border="1">
						...
						<xsl:for-each select="templateBinding">
							...
							<tr>
								<!--
									<td class="packagedElementType">
									<xsl:apply-templates select="./publish:properties/publish:property[@canonicalName='type']"/>
									</td>
								-->
								<td>
									<xsl:call-template	name="outputImageAndTextLinkHTML">
