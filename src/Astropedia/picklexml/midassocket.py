...
serialization to Python 0 or 2""")
            ...
            # OpenContainers, can't handle
            raise Exception("""
Can't handle OpenContainers binary serialization from Python: Reconnect and seed
the conversation (be the first to send a message) so you can set the
serialization to Python 0 or 2""")
        else :
    ...
    def readExact_(self, fd, bytes):
        ...
        final_buff    = ''
        while bytes_to_read :
            t = fd.recv(bytes_to_read) # socket's recv, not ours!
            if t == "": retries-=1
            if retries<0 :
