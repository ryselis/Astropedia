...
try :
    ...
    some_eval = safe_eval
    some_eval('1+2j')   # If we can't handle complexes, have to use eval
except :
...
# What do we support? prefer numpy (don't even import Numeric if we have numpy)
...
DispositionMap = { ARRAYDISPOSITION_AS_NUMERIC         : OCToNumericMap,
                   ARRAYDISPOSITION_AS_LIST            : OCToNumericMap,
                   ARRAYDISPOSITION_AS_NUMPY           : OCToNumPyMap,
                   ARRAYDISPOSITION_AS_PYTHON_ARRAY    : OCToNumericMap,
                   ARRAYDISPOSITION_AS_NUMERIC_WRAPPER : OCToNumericMap,
                  }
...
class XMLLoaderA(object) :
  ...
  def __init__ (self, reader, options,
      ...
      *The array_disposition tells what to do with Numeric arrays:
       ARRAYDISPOSITION_AS_LIST turns them into lists, whereas
       ARRAYDISPOSITION_AS_NUMERIC, ARRAYDISPOSITION_AS_NUMPY and
       ARRAYDISPOSITION_AS_NUMPY AS_PYTHON_ARRAY
      ...
      *When problems loading, do we output an error to cerr or not."""
      ...
      if array_disposition==ARRAYDISPOSITION_AS_NUMERIC :
          ...
          # If we support NumPy, immediately construct and use those
          import numpy
          self.array_type = type(numpy.array([],'i'))
  ...
  def _postProcessListsOflist (self, child) :
      ...
      if type(child)==self.array_type :
          ...
          elif self.arrayDisp_ == ARRAYDISPOSITION_AS_NUMPY :
              return child
  ...
  def _addToList (self, content, output) :
    ...
    # Assertion: Have a Numeric or NumPy array now
    # Array<T>& l = output;   
    ...
    if (type(nested) in [list, self.array_type]) :
      if (self.arrayDisp_ == ARRAYDISPOSITION_AS_NUMPY) :
          test = output.dtype != nested.dtype 
      elif self.arrayDisp_ == ARRAYDISPOSITION_AS_NUMERIC :
          test = output.typecode() != nested.typecode() 
      else :
          test = output.typecode != nested.typecode 
      if test :
    ...
    if self.arrayDisp_ == ARRAYDISPOSITION_AS_NUMPY :
        # Sigh, NUMPY doesn't support concatenate OR append
        shape = output.shape
        length = shape[0]
        output = numpy.resize(output, (length+1,))
        output[length] = t
    elif self.arrayDisp_ == ARRAYDISPOSITION_AS_NUMERIC:
        import Numeric
    ...
    ##print >> sys.stderr, "************* t is ", t, ' nested is ', nested, '   output is', output
        ...
        output.append(t) ### NO APPEND on Numeric or NumPy Arrays??????
  ...
  def _handlePODArrays (self, name_attrs_content, output) :
    ...
    map = DispositionMap[self.arrayDisp_]
    if val_tag in map :
      return self._handlePODArraysHelper(output, name_attrs_content, map[val_tag])
  ...
  def _createPODArray (self, value, tag, appending) :
    if tag in DispositionMap[self.arrayDisp_] :
      typecode = DispositionMap[self.arrayDisp_][tag]
      if appending :
  ...
  def _getElementName (self) :
      ...
      if (c.isdigit()) :
          if self.options_ & XML_DIGITS_AS_TAGS :
              pass  # Just allow it
          elif self.options_ & XML_TAGS_ACCEPTS_DIGITS == 0:
              self._syntaxError("element names can't start with '"+str(c)+"'")
          else :
              name.append('_')
...
import cStringIO
def ReadFromXMLString (xmlstring,
                       options = XML_STRICT_HDR | XML_LOAD_DROP_TOP_LEVEL | XML_LOAD_EVAL_CONTENT, # best options for invertibility 
                       array_disposition = ARRAYDISPOSITION_AS_NUMERIC_WRAPPER,
                       prepend_char=XML_PREPEND_CHAR) :
    """ Read XML from a file and return it as a dictionary (as approp.)
    ...
    Although AS_NUMERIC_WRAPPER is less compatible, you are not going to lose
    any information."""
    stream_thing = cStringIO.StringIO(xmlstring)
    return ReadFromXMLStream(stream_thing, options, array_disposition, prepend_char)
