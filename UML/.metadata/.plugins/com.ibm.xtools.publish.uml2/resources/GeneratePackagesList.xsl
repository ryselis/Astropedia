...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="packagedElement[@xsi:type='uml:Package'] | uml:Model | uml:Profile" mode="createPackageLists">
        ...
        <xsl:if test="element-available('redirect:open')">
        	<redirect:open file="{$packageListFullPathName}" />
        </xsl:if>
        ...
        <xsl:if test="element-available('redirect:close')">
        	<redirect:close file="{$packageListFullPathName}" />
        </xsl:if>
        ...
        -->
        <xsl:if test="element-available('redirect:open')">
        	<redirect:open file="{$topSummaryFullPathName}" />
        </xsl:if>
        ...
        <xsl:if test="element-available('redirect:close')">
        	<redirect:close file="{$topSummaryFullPathName}" />
        </xsl:if>
    </xsl:template>
