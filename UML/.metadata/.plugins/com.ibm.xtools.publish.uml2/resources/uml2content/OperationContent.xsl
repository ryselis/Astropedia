...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:variable name="operationsExceptionTitle">
        <xsl:call-template name="getLocalizedString">
            <xsl:with-param name="key" select="'ContentHeading.RaisedException'"/>
        </xsl:call-template>
    </xsl:variable> 
					...
					<xsl:for-each select="ownedOperation">
						...
						<dl>
							...
							<xsl:call-template name="getParameters" >
								<xsl:with-param name="paramList" select="ownedParameter[@direction != 'return']"/>
							</xsl:call-template>
							...
							<dt>
								<a>
									<xsl:attribute name="name">
										<xsl:value-of select="concat('operationConstraintsTable', $constraintID)" />
									</xsl:attribute>
									<b>
							...
							<dt>
								<b><xsl:copy-of select="$operationsExceptionTitle" /></b>
							</dt>
							<xsl:call-template name="showRaisedExceptions" />							
		...
		<xsl:if test="count(ancestor) &gt; 0">
			<xsl:if test="$mode = 'summary'">
				<xsl:for-each select="ancestor">
					<xsl:if test="count(inheritedOperation) &gt; 0">
						<p/>
						<table class="SummaryTable" border="1">
							<tr>
								<td class="SummaryTitle" colspan="3">
				...
				<xsl:for-each select="$paramList">
					...
					<tr valign="top">
						...
						<td>
							<xsl:text> [</xsl:text>
							<xsl:value-of 
								select="publish:properties/publish:property[@canonicalName='Multiplicity']/@publish:value" />
							<xsl:text>]</xsl:text>
						</td>
						<td>
							<!-- Documentation -->
	...
	<xsl:template name="showRaisedExceptions" match="ownedOperation">
		<xsl:if test="count(raisedException) &gt; 0">
			<dd>
				<table>
					<xsl:for-each select="raisedException">
						<tr>
             				<xsl:call-template name="createLinkToElement">
