...
<xsl:stylesheet version="1.0" exclude-result-prefixes="uml xsi publish"
    ...
    <xsl:template name="SplitMode_createAPackageElementList">
        ...
        <xsl:if test="element-available('redirect:open')">
        	<redirect:open file="{$packageNavigationListFullPathName}" />
        </xsl:if>
        ...
        <xsl:if test="element-available('redirect:close')">
        	<redirect:close file="{$packageNavigationListFullPathName}" />
        </xsl:if>

        <!-- Create the package content file and fill it with header information -->
        <!-- commented out for XSLTC -->
        <xsl:if test="element-available('redirect:open')">
        	<redirect:open file="{$packageContentFullPathName}" />
        </xsl:if>
        ...
        <xsl:if test="element-available('redirect:close')">
        	<redirect:close file="{$packageContentFullPathName}" />
        </xsl:if>
    </xsl:template>
